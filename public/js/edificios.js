...
/**
 *    * Evento para agregar una lectura inicial
 *       */
  $(document).on('click', '#updateLecturaInicial', function (event) {
    event.preventDefault();

    $('#modal-lectura-inicial').modal('show');

  });

    /**
 *    * Evento para guardar una lectura inicial
 *       */
    $(document).on('click', '#saveLecturaInicial', function (event) {
        event.preventDefault();

        var formData = new FormData(document.getElementById("lecturaInicialForm"));

        var admigas_departamentos_id = $("#idSeleccionado").val();
        var lectura = $("#lecturaInicial").val();
        var fecha = $("#fechaLectura").val();
        var testigo = $("#testigoFotografico").prop('files')[0];

        console.log(testigo);

        var _token = $("input[name=_token]").val();

        formData.append("admigas_departamentos_id", admigas_departamentos_id);
        formData.append("lectura", lectura);
        formData.append("fecha", fecha);
        formData.append("testigo", testigo);
        formData.append("_token", _token);

        var url = currentURL + '/captura-lectura-inicial';
        $.ajax({
          url: url,
          type: 'POST',
          data: formData,
          ...
          success: function success(result) {
            ...
            $('#modal-lectura-inicial').modal('hide');
            $('.modal-backdrop ').css('display', 'none');

            Swal.fire('Confirmado!', 'La lectura ha sido guardada.', 'success');
...
$(function () {
  ...
  /**
   * Evento para ver las fotos de las lecturas
   */
