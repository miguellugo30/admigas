...
class QuerysJoinController extends Controller
{
    ...
    public function queryExcelLecturas($id)
    {
        ...
        ->select(
            ...
            DB::raw("(SELECT admigas_lecturas_medidores.lectura FROM admigas_lecturas_medidores WHERE admigas_lecturas_medidores.admigas_medidores_id = admigas_medidores.id AND admigas_lecturas_medidores.activo = 1 ORDER BY fecha_lectura DESC LIMIT 1 ) AS lectura_anterior"),
        )
            ->where('admigas_departamentos.admigas_condominios_id', $id)
            ->where('admigas_departamentos.activo', 1)
            ->where('admigas_medidores.activo', 1)
    ...
    public function calcularConsumos($deptos, $condominio, $empresa_id )
    {
	    ...
	    /**
             * Validamos que factor se usara
             * Si el condominio es punto de venta (1) se usara el factor del departamento
             * Si no, se usara el factor del condominio
             */
            if ($condominio->first()->tipo == 1) {
                $factor = $depto->factor;
            } else {
		...
		$depto->me3 = ( ($depto->lectura_actual+9999.99) - $depto->lectura_anterior );
	    }
            else {
		$depto->me3 = ( $depto->lectura_actual - $depto->lectura_anterior );
            ...
            $depto->consumo = round( ( $depto->litros ) * ( $precio - $condominio->first()->descuento ) );
    ...
    public function generarRecibos($deptos, $condominio, $empresa_id, $fecha_recibo, $mensaje)
    {
	    ...
	    /**
            ...
            if ($depto->gasto_admin != null)
            {
                $gasto_admin = $depto->gasto_admin ;
            }
            else
            {
                $gasto_admin = $condominio->gasto_admin ;
            }
	    ...
	    /**
            ...
            if($depto->lectura_actual < $depto->lectura_anterior) {
                $importe = round( ( ( ($depto->lectura_actual+9999.99) - $depto->lectura_anterior ) * $factor ) * ( $precio - $condominio->descuento ) );
            }
            else {
                $importe = round( ( ( $depto->lectura_actual - $depto->lectura_anterior ) * $factor ) * ( $precio - $condominio->descuento ) );
            }
	    ...
	    $v['clave_recibo'] = "A-".( $consecutivo++ );
            ...
            $v['importe'] = $importe;
            $v['gasto_admin'] = $gasto_admin;
            $v['adeudo_anterior'] = $adeudo;
            $v['cargos_adicionales'] = $cargos_adicionales;
            $v['total_pagar'] =$cargos_adicionales + $adeudo + $gasto_admin + $importe;
	    ...
	    if($depto->lectura_actual < $depto->lectura_anterior) {
                $litros = ( ($depto->lectura_actual+9999.99) - $depto->lectura_anterior ) * $condominio->factor;
            }
            else {
                $litros = ( $depto->lectura_actual - $depto->lectura_anterior ) * $condominio->factor;
            }
