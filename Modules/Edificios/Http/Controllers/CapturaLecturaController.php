...
class CapturaLecturaController extends Controller
{
    ...
    public function syncData(Request  $request)
    {
        ...
        if ($request->admigas_condominios_id == 116 || $request->admigas_condominios_id == 117 || $request->admigas_condominios_id == 118) {
			$fecha = $request->fecha_lectura;
        } else {
            $fecha = date('m-Y', strtotime($request->fecha_lectura));
        }
         ...
         */
        $this->donwload->donwloadFotos( $condominio, $this->empresa_id , $fecha);
        /**
         * Descargamos las lecturas y obtenemos la informacion
         */
        if ( !$this->donwload->importLecturas($condominio, $this->empresa_id, $fecha) ) {
            ...
            $data = $this->donwload->importLecturas($condominio, $this->empresa_id, $fecha);
        }
        /**
         * Obtenemos los lecturistas de la empresa
         */
    ...
    public function capturaLecturaInicial(Request $request){

        /**
         * Obtenemos departamento con condominios
         */
        $depto = AdmigasDepartamentos::with('Condominios')
                                        ->where('id', $request->admigas_departamentos_id)
                                        ->first();
        /**
         * Obtenemos el id medidos
         */
        $idMedidor = AdmigasMedidores::where('admigas_departamentos_id', $request->admigas_departamentos_id)
                    ->first();

        $fecha_lectura = date('m-Y', strtotime($request->fecha));

        $nombreImagen = $request->admigas_departamentos_id."_".$depto->numero_departamento.".jpeg";

        //dd($request->file('testigo'));
        $request->file('testigo')->storeAs('/1\/' . $depto->Condominios->id . '\/' . $fecha_lectura . '\/',$nombreImagen );

        //Storage::put('/1\/' . $depto->Condominios->id . '\/' . $fecha_lectura . '\/' . $nombreImagen, $request->testigo);

        DB::table('admigas_lecturas_medidores')->insert([
            'lectura' => $request->lectura,
            'fecha_lectura' => $request->fecha,
            'admigas_departamentos_id' => $request->admigas_departamentos_id,
            'admigas_medidores_id' => $idMedidor->id
        ]);

        /**
         * Redireccionamo
         */
         return redirect()->route('condominios.show', [$depto->Condominios->id]);
