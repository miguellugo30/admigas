<div class="card-header">
    <h3 class="card-title">
      ...
      <div class="card-tools">
          ...
          <input type="hidden" name="admigas_condominios_id" id="admigas_condominios_id" value="{{ $condominio->id }}">
    </div>
</div>
<div class="card-body">
    <div class="row">
            ...
            <tbody>
                @for ($i = 0; $i < count( $deptos ); $i++)
                    ...
                    <tr data-id='{{ $depto->departamento_id }}' class="text-center">
                        ...
                        <td>
                            <input type="text" name="lectura" id="lectura_{{ $i }}" class="nueva_lectura form-control form-control-sm" data-posicion="{{ $i }}" data-lectura_anterior="{{ $depto->lectura_anterior }}" size="10" value="{{ $data[$i][4] }}">
                        ...
                        <td class="diferencia_{{ $i }}">
                            {{ $data[$i][4] - $depto->lectura_anterior }}
                        </td>
                        <td>
                            @if ($condominio->id == 116 || $condominio->id == 117 || $condominio->id == 118)
                                @php
                                    $fecha = $fecha_lectura;
                            ...
                            @else
                                @php
                                    $fecha = date('m-Y', strtotime($fecha_lectura));
                                @endphp
                            @endif

                            <img src="{{ \Storage::url( $empresa_id.'/'.$condominio->id.'/'.$fecha.'/'.$depto->departamento_id."_".$depto->numero_departamento.".jpeg" ) }}" alt="" width="140px">
                        </td>
                    </tr>
                @endfor
            </tbody>
