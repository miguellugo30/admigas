...
<div class="card-body">
                ...
                @for ($i = 0; $i < count( $data ); $i++)
                    ...
                    <tr data-id='{{ $depto->departamento_id }}' class="text-center">
                        ...
                        <td
                            ...
                            class="viewPhoto"
                            data-photo="storage/1/{{$condominio->first()->id}}/{{ date("m-Y", strtotime($depto->fecha_lectura_anterior)) }}/{{ $depto->departamento_id }}_{{ $depto->numero_departamento }}.jpeg"
                        >
                            {{ $depto->lectura_anterior }}
                        </td>
                        <td
                            style="cursor: pointer"
                            class="viewPhoto"
                            data-photo="storage/1/{{$condominio->first()->id}}/{{ date("m-Y", strtotime($depto->fecha_lectura_actual)) }}/{{ $depto->departamento_id }}_{{ $depto->numero_departamento }}.jpeg"
                        >
                            {{ $depto->lectura_actual }}
                        </td>
 ...
 <!-- MODAL -->
 <div class="modal fade bd-example-modal-lg" tabindex="-1" id="modal-photo" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog " role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="tituloModal">Testigo Fotográfico</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="modal-body">
                <img src="" alt="testigo-fotografico" id="photo" class="img-fluid">
            </div>
